...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const form = document.getElementById('registerRecepForm');
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await apiFetch('/users/register-receptionist', {
                method: 'POST',
                body: formData
            });
            ...
            if (response.ok && result.success) {
                ...
                if (result.errors && Array.isArray(result.errors)) {
                    ...
                    if (!hasFieldErrors) {
                        showError(result.message || 'Error al registrar el recepcionista.');
                    }
