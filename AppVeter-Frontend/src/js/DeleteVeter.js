...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tbody = document.getElementById('veterinarians-tbody');
    ...
    async function loadVeterinarians() {
        try {
            const response = await apiFetch(apiUrl('/users/veterinarians', { page: 1, limit: 50 }));
            ...
            if (response.ok && result.success) {
                ...
                if (users.length === 0) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="6" class="loading-td">No hay veterinarios registrados.</td>
                        </tr>
                    `;
                    return;
                }
                    ...
                    <tr>
                        ...
                        <td class="actions-col">
                            <div class="actions-wrapper">
                                <button class="action-btn btn-delete" data-id="${escapeAttr(user.id)}" data-name="${escapeAttr(user.name || '')} ${escapeAttr(user.lastname || '')}" title="Eliminar veterinario">
                ...
                document.querySelectorAll('.btn-delete').forEach(btn => {
                    btn.addEventListener('click', async () => {
                        ...
                        const confirmed = confirm(`¿Estás seguro de eliminar al veterinario "${name}"? Esta acción no se puede deshacer.`);
                        ...
                        try {
                            ...
                            if (delResponse.ok && delResult.success) {
                                ...
                                alert(delResult.message || 'Error al eliminar el veterinario.');
                            }
                        } catch (error) {
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="6" class="loading-td">Error al cargar los veterinarios.</td>
                    </tr>
                `;
            }
        } catch (error) {
